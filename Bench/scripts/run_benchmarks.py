...
# 添加Bench目录到Python路径，确保可以导入本地模块
bench_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, bench_dir)

# 导入路径配置
from src.path_config import get_benchmark_path, ensure_directories

import pandas as pd

from src.abstractions import BenchmarkCircuit, SimulatorInterface
from src.caching import CacheConfig, create_cache_instance
from src.post_processing import (analyze_results,
                                               generate_summary_report)
from src.simulators import QiboWrapper
...
def create_simulator_instances(
    ...
    Note:
        ...
        - 对于其他平台，尝试从"src.simulators.{platform}_wrapper"模块导入
    ...
    for config in simulator_configs:
        ...
        try:
            if platform == "qibo":
                ...
                # 尝试动态导入其他平台的包装器类
                module_name = f"src.simulators.{platform}_wrapper"
...
def create_circuit_instances(circuit_names: List[str]) -> List[BenchmarkCircuit]:
    ...
    Note:
        动态导入规则：
        - 模块路径：src.circuits.{circuit_name}
        - 类名规则：
    ...
    for circuit_name in circuit_names:
        try:
            # 动态导入电路模块
            module_name = f"src.circuits.{circuit_name}"
            module = importlib.import_module(module_name)
...
def setup_output_directory(args) -> Path:
    ...
    # 使用统一的路径配置
    if args.output_dir == "results":  # 默认值
        output_dir = get_benchmark_path(timestamp, create=True)
    else:
        # 自定义输出目录
...
def initialize_components(args):
    ...
    return simulators, circuits


def setup_cache_configuration(args):
    """设置缓存配置
...
def execute_benchmark_suite(circuits, simulators, args, cache_config):
    """执行基准测试套件
...
def display_cache_statistics(cache_config):
    """显示缓存统计信息
...
def prepare_summary_data(results, repeat):
    """准备摘要数据
...
def process_results(results, output_dir, args):
    """处理和保存基准测试结果
    ...
    try:
        ...
        # 准备数据用于生成摘要报告
        data = prepare_summary_data(results, args.repeat)
...
def print_startup_info(output_dir, args):
    """打印启动信息
...
def main() -> int:
    ...
    Note:
        这个函数协调整个基准测试流程，包括参数解析、组件初始化、
        基准测试执行和结果处理。所有具体的操作都委托给专门的函数。
    """
    global args
    ...
    try:
        ...
        print_startup_info(output_dir, args)
        ...
        # 步骤6: 显示缓存统计信息
        if args.cache_stats:
            display_cache_statistics(cache_config)
