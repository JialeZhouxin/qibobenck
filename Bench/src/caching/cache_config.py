...
# 导入路径配置
try:
    from ..path_config import get_cache_path
except ImportError:
    # 如果导入失败，使用默认路径
    def get_cache_path(cache_type="hybrid", create=True):
        return Path(f".benchmark_cache/{cache_type}")
...
class CacheConfig:
    ...
    disk_cache_dir: str = str(get_cache_path("disk", create=True))
